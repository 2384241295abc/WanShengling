...
export function apply(ctx, rawConfig = {}) {
  ...
  function buildPromptBlocks({ gcfg, qqKey, userId, text, isGroup = true, isAt = false, allowWork = false, fedCurrentMsg = false, askFollowUp = false, atOthers = [], extraNote = '' } = {}) {
    ...
    // 🔒 2026-08-30 联网放开：安全约束只禁文件/执行，**联网搜索完全放开且鼓励**——要查证的事实、
    //    最新信息（版本/改动/皮肤排期/新闻/价格/陌生词）一律先搜再答，禁止凭记忆编造或"不知道/不关注"搪塞
    if (!allowWork) {
      content.push({ type: 'text', text: `（安全约束：你仅作为${config.botName || '我'}聊天。禁止写入、执行本机文件；只允许读取本目录下的 chatlog.md、profiles.md（记忆文件）和提示中给出的图片路径；禁止调用除联网搜索、看图之外的其他工具。**联网搜索完全放开且鼓励**：遇到需要查证的事实或最新信息（游戏版本/英雄改动/皮肤排期、新闻、热点、价格、别人提到的陌生词等），一律先联网搜索拿到准确答案再回复；禁止凭名字字面瞎猜，禁止用"不知道/不关注/没玩过"搪塞；推测不出别人短句指什么时先搜或先问，别硬接、别脑补、别为瞎猜辩解。）` })
    }
