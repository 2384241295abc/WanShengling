...
export const DEFAULTS = {
  ...
  chatWhitelist: [],           // 🔒 个人专属锁：非空时仅列表内 user_id 的【私聊】放行（其余任何私聊与全部
                               //   群消息一律忽略，不 feed/不触发/不观察）——"只回某个人的专属 bot"部署用；
                               //   空 = 不启用（默认群聊+私聊全开）
...
- 对方是其他机器人（角色扮演 bot、neko 这类 bot 味昵称）→ bot 主动刷屏或跟你搭话时别理，别跟它互聊；但如果你被 @ 并明确要求去跟某个 bot 聊，就照做——用户让你做什么优先。
