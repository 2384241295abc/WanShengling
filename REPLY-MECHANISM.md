...
> `discussion.mjs`（讨论模式）、`reply-buffer.mjs`（回复缓冲）、`session.mjs`（会话管理）。
...
| `friends.checkSettle` | 万生玲发言后满 5 句 → 结算窗口内成员 +1 |
...
- **续期**：发起人友好度上升（`add()` 内）→ 刷新计时。
- **退出**：`checkSolosExpiry` 每 10s 检查，发起人友好度超过 `soloIdleMs`（默认 60s）未上升即退出。
...
| 顺序 | 块 | 说明 |
|------|-----|------|
| 1 | 人设 | 补丁 `persona`（v4.0 精简版） |
| 2 | 成员认知 | 群聊：昵称/印象/发言次数 |
| 3 | 讨论环境 | 仅讨论模式激活时注入 |
| 4 | 友好度认知 | 当前说话者的熟悉度与语气提示（群+私聊都注入） |
| 5 | 能量上下文 | 群聊：最近 `contextWindow` 条聊天记录（含万生玲自己上一条，标"万生玲:"） |
| 6 | 用户文本 | 或纯 @ 时的"（对方@了你）" |
