...
DeepSeek Harness × QQ 远程交互桥（OneBot 11）—— 独立 Cordis 插件包（v0.3.4）。
...
| **能量节奏** | 群聊不是每条都回：回复后能量随机 `[30,90]`，每分钟衰减 3，每条消息扣 10(挚友 17)，`能量<0` 才回；@ 必回 |
| **回复冷却** | 回复后 15s 忙期：忙期内所有消息(含 @)只缓冲扣能入历史、不触发；到期消息驱动触发，冷群靠衰减或「CD 到期补回」自动回一条(2026-08-30)；入队即忙期防连发(2026-09-04 单窗口整合 inFlight) |
